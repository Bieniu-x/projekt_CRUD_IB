...
// pobieranie tokenu z cookie lub nagłówka Bearer
  ...
  try {
    ...
    if (existingUsername) {
      return res.status(409).json(
        errorResponse(409, [
          { field: 'username', code: 'DUPLICATE', message: 'Nazwa użytkownika już zajęta' },
        ])
      );
    }
    ...
    if (existingEmail) {
      return res.status(409).json(
        errorResponse(409, [
          { field: 'email', code: 'DUPLICATE', message: 'Email jest już zarejestrowany' },
        ])
      );
    }
    ...
    if (
      ...
      return res
        .status(409)
        .json(errorResponse(409, [], 'Duplikat danych - użytkownik już istnieje'));
    }
...
router.post('/login', async (req, res) => {
  ...
  if (!username || !password) {
    return res.status(400).json(
      errorResponse(400, [], 'Podaj nazwę użytkownika i hasło')
    );
  }
  ...
  try {
    ...
    if (!user) {
      return res
        .status(401)
        .json(errorResponse(401, [], 'Nieprawidłowe dane logowania'));
    }
